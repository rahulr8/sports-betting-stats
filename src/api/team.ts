...
export const fetchMatchesForTeam = async (teamName: TeamCodeKey): Promise<Match[]> => {
  try {
    ...
    const homeMatchesQuery = query(collection(db, "england-dump"), where("homeTeam", "==", teamName));
    const awayMatchesQuery = query(collection(db, "england-dump"), where("awayTeam", "==", teamName));
